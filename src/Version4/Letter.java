...
public class Letter {
    ...
    public static final double LETTERS_LFT_X = OmegaFight3.NORM_SCREEN_CENTER.x - (OmegaFight3.TITLE_SIZE.x) / 2;
    ...
    public void draw(Graphics g) {
        ...
        if (isVertical) {
            curSize = new Coord(size.x * OmegaFight3.lerp(LETTER_MIN_SIZE, 1.0, progress), OmegaFight3.lerp(OmegaFight3.NORM_SCREEN_SIZE.y, size.y, progress));
        }

        // Horizontal blip calculation
        else {
            curSize = new Coord(OmegaFight3.lerp(OmegaFight3.NORM_SCREEN_SIZE.x, size.x, progress), size.y * OmegaFight3.lerp(LETTER_MIN_SIZE, 1.0, progress));
        }

        // Drawing
        Coord drawCoord = coord.add(curSize.scaledBy(-0.5));
        g.drawImage(image, OmegaFight3.coordToScreenX(drawCoord.x), OmegaFight3.coordToScreenY(drawCoord.y), OmegaFight3.sizeToScreenX(curSize.x), OmegaFight3.sizeToScreenY(curSize.y), null);
    }
}
