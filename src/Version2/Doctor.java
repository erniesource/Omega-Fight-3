...
public class Doctor extends Boss {
    ...
    public static final int SPIT_HZ = 25;
    public static final Coord COORD_TO_SPIT_COORD = new Coord(STATE_SIZE[SPIT].x / 4, STATE_SIZE[SPIT].y * 3 / 10);

    // Laugh constants
    public static final int LAUGH_NO_PROJS = 10;
    public static final int LAUGH_HZ = 20;
    ...
    public static final double PINCER_AMT_SCALING_TO_HEALTH = 0.5;
    public static final int PINCER_HZ = 180;
    public static final double BOMBOT_AMT_SCALING_TO_HEALTH = 0.25;
    public static final int BOMBOT_HZ = 300;
    public static final int BOMBOT_NUM_SPAWN_LOCS = 4;
    public static final int DOWN_SPAWN = 0;
    public static final int LEFT_SPAWN = 1;
    public static final int TOP_SPAWN = 2;
    public static final int RIGHT_SPAWN = 3;
    ...
    public void backgroundAttack() { // DOESN'T WORK
        pincerCounter++;
        if (pincerCounter >= PINCER_HZ * health / (INITIAL_HEALTH * difficulty) / PINCER_AMT_SCALING_TO_HEALTH) {
        ...
        bombotCounter++;
        if (bombotCounter >= BOMBOT_HZ * health / (INITIAL_HEALTH * difficulty) / BOMBOT_AMT_SCALING_TO_HEALTH) {
            int spawn = (int) (Math.random() * BOMBOT_NUM_SPAWN_LOCS);
            if (spawn == DOWN_SPAWN) {
                projectiles.add(new Bombot(this, new Coord(OmegaFight3.SCREEN_SIZE.x / 2, OmegaFight3. SCREEN_SIZE.y), -Math.PI / 2, OmegaFight3.LEFT_SIGN));
            }
            else if (spawn == LEFT_SPAWN) {
                projectiles.add(new Bombot(this, new Coord(0, OmegaFight3.SCREEN_SIZE.y / 2), 0, OmegaFight3.RIGHT_SIGN));
            }
            else if (spawn == TOP_SPAWN) {
                projectiles.add(new Bombot(this, new Coord(OmegaFight3.SCREEN_SIZE.x / 2, 0), Math.PI / 2, OmegaFight3.RIGHT_SIGN));
            }
            else if (spawn == RIGHT_SPAWN) {
                projectiles.add(new Bombot(this, new Coord(OmegaFight3.SCREEN_SIZE.x, OmegaFight3.SCREEN_SIZE.y / 2), Math.PI, OmegaFight3.LEFT_SIGN));
            }
            bombotCounter = 0;
        }
    }
